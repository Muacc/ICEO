...
<script>
const regionData = require('@/pageCopy/components/biaofun-region/region.json');
export default {
	/**  
	 * 数据  
	 */
	...
	methods: {
		...
		setDefaultValue() {
			...
			this.regionStr =
				...
				// regionData[provinceIndex].children[cityIndex].name;
				regionData[provinceIndex].children[cityIndex].children[areaIndex].name
		},
		...
		change(event) {
			...
			this.regionStr = this.range[2][this.value[2]].name;
			// 传递事件
